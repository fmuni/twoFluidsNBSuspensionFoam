...
void Foam::twoPhaseSystem::solve()
{
    ...
    volVectorField gradAlpha1 = fvc::grad(alpha1,"bounded");
    ...
    if (pPrimeByA_.valid())
    {
        ...
        (
            (
                fvc::interpolate(gradAlpha1)&pPrimeByA_()
            )
          & mesh_.Sf()
        );
    ...
    for (int acorr=0; acorr<nAlphaCorr; acorr++)
    {
        ...
        if (pPrimeByA_.valid())
        {
            ...
            label maxODCorr(
                readLabel(alphaControls.lookup("nAlphaOffDiagCorr"))
            );

            //- Correct for explicit off diag 
            for(label odcorr=0; odcorr<maxODCorr; odcorr++)
            {

                gradAlpha1 = fvc::grad(alpha1,"bounded");
                 ...
                 == 
                    fvc::surfaceIntegrate (
                      ...
                      * (
                          ...
                          & (pPrimeByA_() - tr(pPrimeByA_())*tensor::I)
                        )
                      & mesh_.Sf()
                    )
                );
