...
:
    ...
    suspensionStressModel_
    (
        kineticTheoryModels::suspensionStressModel::New
        (
            coeffDict_
        )
    ),
    nonLocalShearRateModel_
    (
        kineticTheoryModels::nonLocalShearRateModel::New
        (
            coeffDict_
        )
    ),
    ...
    (
        IOobject
        (
            IOobject::groupName("gammaDotEff", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("Q", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::MUST_READ,
            IOobject::AUTO_WRITE
...
bool Foam::RASModels::kineticTheoryModel::read()
{
    ...
    )
    {
        ...
        suspensionStressModel_->read();
        nonLocalShearRateModel_->read();
...
) const
{
    ...
    (
      ...
      - rho_*suspensionStressModel_->shearForce
        (
            phase_,
            alphaMax_,
            gammaDotEff_,
...
Foam::tmp<Foam::volSymmTensorField>
Foam::RASModels::kineticTheoryModel::DSigma_sDalpha() const
{
    return
    (
        suspensionStressModel_->DSigmaDalpha
        (
            phase_,
            alphaMax_,
            gammaDotEff_,
...
void Foam::RASModels::kineticTheoryModel::correct()
{
...

    {
        ...
        volScalarField nus = suspensionStressModel_->nu
        (
            phase_,
            alphaMax_
        );

        // Update effective shear rate
        gammaDotEff_ = nonLocalShearRateModel_->gammaDotEff
        (
            phase_,
            Theta_
        );

        // Update fabric tensor
        Q_ = suspensionStressModel_->computeQ(phase_);
        Q_.correctBoundaryConditions();

        // Limit viscosity and add frictional/suspension viscosity
        nut_.min(maxNut_);
        nuFric_ = min(nuFric_, maxNut_ - nut_);
        nut_ += nuFric_;
        nus = min(nus, maxNut_ - nut_);
        nut_ += nus;
